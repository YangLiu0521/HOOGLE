...
<title>修改訂單 - update_ord_input.jsp</title>
...
<table id="table-1">
	<tr><td>
		 <h3>訂單新增 - addOrd.jsp</h3></td><td>
		 <h4><a href="select_ord_page.jsp"><img src="images/logo.png" width="100" height="100" border="0">回首頁</a></h4>
	</td></tr>
</table>
...
<table>
	<tr>
		...
		<td><input type="TEXT" name="userId" size="45" 
			 value="${param.userId}"/></td><td>${errorMsgs.userId}</td>
	</tr>
	<tr>
		<td>飯店編號:</td>
	...
	<tr>
		<td>訂房日期:</td>
		<td><input name="ordDate" id="f_date1" type="text"/></td><td>${errorMsgs.ordDate}</td>
	</tr>
	<tr>
		<td>入住日期:</td>
		<td><input name="ordCheckin" id="f_date2" type="text"/></td><td>${errorMsgs.ordCheckin}</td>
	</tr>
	<tr>
		<td>退房日期:</td>
		<td><input name="ordCheckout" id="f_date3" type="text"/></td><td>${errorMsgs.ordCheckout}</td>
	</tr>
	<tr>
		<td>訂房天數:</td>
...
<% 
  ...
  try {
	  ordDate = java.sql.Date.valueOf(request.getParameter("ordDate").trim());
   } catch (Exception e) {
	  ordDate = new java.sql.Date(System.currentTimeMillis());
   }
%>
<% 
  java.sql.Date checkinDate = null;
  try {
	  checkinDate = java.sql.Date.valueOf(request.getParameter("checkinDate").trim());
   } catch (Exception e) {
...
<% 
  ...
  try {
	  checkoutDate = java.sql.Date.valueOf(request.getParameter("checkoutDate").trim());
   } catch (Exception e) {
...
<script>
        ...
        $('#f_date2').datetimepicker({
 	       ...
 	       step: 1,                //step: 60 (這是timepicker的預設間隔60分鐘)
 	       format:'Y-m-d',         //format:'Y-m-d H:i:s',
 		   value: '<%=checkinDate%>', // value:   new Date(),
            //disabledDates:        ['2017/06/08','2017/06/09','2017/06/10'], // 去除特定不含
            //startDate:	            '2017/07/10',  // 起始日
            //minDate:               '-1970-01-01', // 去除今日(不含)之前
            //maxDate:               '+1970-01-01'  // 去除今日(不含)之後
        ...
        $('#f_date3').datetimepicker({
  	       ...
  	       step: 1,                //step: 60 (這是timepicker的預設間隔60分鐘)
  	       format:'Y-m-d',         //format:'Y-m-d H:i:s',
  		   value: '<%=checkoutDate%>', // value:   new Date(),
             //disabledDates:        ['2017/06/08','2017/06/09','2017/06/10'], // 去除特定不含
             //startDate:	            '2017/07/10',  // 起始日
             //minDate:               '-1970-01-01', // 去除今日(不含)之前
             //maxDate:               '+1970-01-01'  // 去除今日(不含)之後
